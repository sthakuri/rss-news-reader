...
namespace NewsFeed.Extension
{
    ...
    {
        public static string ToLovelyTime(DateTime dt)
        {
            const int SECOND = 1;
        ...
        public static DateTime ToDateTime(string strDateTime)
        {
            ...
            catch
            {
                try
                {
                    //Thu, 18 Feb 2016 17:00:06 PST

                    strDateTime = strDateTime
                        .Replace(GetTzAbbreviation(TimeZone.CurrentTimeZone.StandardName), "")
                        .Replace(GetTzAbbreviation(TimeZone.CurrentTimeZone.DaylightName), "");
                    dt = DateTime.Parse(strDateTime);
                }
                catch (Exception ex)
                {
